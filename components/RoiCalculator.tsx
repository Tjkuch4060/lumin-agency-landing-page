    ...
    <>
      ...
      <section className="bg-slate-900 py-20 relative overflow-hidden" id="roi-calculator">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700 rounded-2xl shadow-2xl overflow-hidden grid md:grid-cols-2 min-h-[600px] roi-calculator">
            {/* --- INPUTS --- */}
            <div className="p-6 md:p-10 text-white">
              <div className="grid gap-6">
                ...
                <div className="slider-container">
                  ...
                  <input type="range" id="automation-percentage" min="30" max="90" value={automationPercentage} onFocus={handleStart} onChange={(e) => { setAutomationPercentage(parseInt(e.target.value)); setActivePreset(null); setHasInteracted(true); }} className="w-full h-3 bg-slate-700 rounded-lg appearance-none cursor-pointer range-thumb" />
            ...
            {/* --- RESULTS --- */}
            {hasInteracted ? (
              <div className="p-6 md:p-10 bg-slate-900/50 border-l border-slate-700">
                <div className="mb-6">
                  <h3 className="text-2xl md:text-3xl font-bold text-white">Your Potential Savings</h3>
                </div>
                <div className="grid grid-cols-1 gap-5 mb-6">
                  <div className="bg-gradient-to-br from-blue-500 to-blue-700 text-white p-6 rounded-2xl text-center">
                    <div className="text-3xl md:text-4xl font-extrabold"><AnimatedNumber value={results.annualSavings} formatter={formatCurrency} /></div>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    {[
                      ...
                      <div key={res.label} className="bg-slate-800 p-4 rounded-xl">
                        <div className="text-xl md:text-2xl font-bold text-white"><AnimatedNumber value={res.value} formatter={res.formatter} /></div>
                        <div className="text-xs md:text-sm text-slate-400 mt-1">{res.label}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-slate-700 text-center">
                  <h4 className="text-lg md:text-xl font-bold text-white mb-4">Ready to realize these savings?</h4>
